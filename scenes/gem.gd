...
func _ready() -> void:
    ...
    tween.set_loops()
    tween.tween_property($"Sprite2D", "position", Vector2(0, -4), 0.5).set_trans(Tween.TRANS_CUBIC)
    tween.tween_property($"Sprite2D", "position", Vector2(0, 0), 0.5).set_trans(Tween.TRANS_CUBIC)
